...
const amSlipUpdate = createRequestSaga(types.UPDATE_AM_SLIP_REQUEST,api.amSlipUpdate);
...
export default function* AccountSaga() {
    ...
    yield takeLatest(types.SELECT_SLIP_START, selectSlipSaga); //전표조회
    yield takeLatest(types.DELETE_SLIP_START, deleteSlipSaga); //전표삭제
    yield takeLatest(types.UPDATE_SLIP_START, updateSlipSaga); //전표 업데이트
    yield takeLatest(types.SELECT_JOURNAL_START, searchJournalSaga); //분개조회
    yield takeLatest(types.DELETE_JOURNAL_START, deleteJournalSaga); //분개삭제
    yield takeLatest(types.SAVE_JOURNAL_START, saveJournalSaga); //분개저장 insert
    yield takeLatest(types.UPDATE_JOURNAL_START, updateJournalSaga); //분개저장 update
    yield takeLatest(types.SELECT_JOURNAL_DETAIL_START, searchJournalDetailSaga); //분개상세 조회
    yield takeLatest(types.SAVE_JOURNAL_DETAIL_START, saveJournalDetailSaga); //분개상세저장
    ...
    yield takeLatest(types.UPDATE_AM_SLIP_REQUEST, amSlipUpdate);
    
    yield takeLatest(types.SET_JOURNAL_NO_REQUEST, getJournalNo); // //*********** 2020-08-28 정대현 추가 **********
    yield takeLatest(types.SELECT_GENERAL_ACCOUNT_LEDGER_START,selectGeneralAccountLedgerSaga,);
     //************************* 2020-12-04 분개장 시작 *************************
    yield takeLatest(types.SEARCH_JOURNAL_DOUBLE_REQUEST, searchJournalDoubleSaga);
    //************************* 2020-12-04 분개장 종료 *************************

    //고장자산 수정 삭제 조회 등록 박미노==========================================
    yield takeLatest(types.SEARCH_NON_CURRENT_REQUEST, selectNonCurrentSaga);
    yield takeLatest(types.SAVE_NON_CURRENT_START, saveNonCurrentSaga);
    yield takeLatest(types.DELETE_NON_CURRENT_START, deleteNonCurrentSaga);
}
